...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        ...
        data = spacex_df[spacex_df['Launch Site'] == selected_site]['class'].value_counts().reset_index()
        data.columns = ['class', 'count']
        fig = px.pie(data, values='count', names='class', title=f'Success vs Failed for {selected_site}')
    return fig
...
def update_scatter(selected_site, payload_range):
    ...
    fig = px.scatter(
        ...
        title='Payload vs. Outcome for {}'.format(selected_site if selected_site != 'ALL' else 'All Sites')
    )
    return fig
